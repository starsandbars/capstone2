    ...
    var worriedSymptomCard: some View {
        if let worried = summary.mostWorriedSymptom {
            GlanceCard(
                ...
                label: NSLocalizedString(worried.name, comment: worried.name),
            ...
            if summary.mostCommonSymptoms.isEmpty {
                ...
                VStack(spacing: 10) {
                    ForEach(Array(summary.mostCommonSymptoms.enumerated()), id: \.offset) { index, symptom in
                        FrequencyRow(
                            rank: index + 1,
                            name: NSLocalizedString(symptom.name, comment: symptom.name),
    ...
    var mentalHealthSection: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            VStack(spacing: 12) {
                EmotionalSummaryRow(
                    icon: "flame.fill",
                    label: NSLocalizedString("home.emotion.anger",     comment: ""),
                    score: summary.averageAnger,
                    color: Color(hex: "E74C3C")
                )
                EmotionalSummaryRow(
                    icon: "cloud.rain.fill",
                    label: NSLocalizedString("home.emotion.anxiety",   comment: ""),
                    score: summary.averageAnxiety,
                    color: Color(hex: "5B85C4")
                )
                EmotionalSummaryRow(
                    icon: "person.fill.xmark",
                    label: NSLocalizedString("home.emotion.loneliness",comment: ""),
                    score: summary.averageLoneliness,
                    color: Color(hex: "8E44AD")
                )
                EmotionalSummaryRow(
                    icon: "scalemass.fill",
                    label: NSLocalizedString("home.emotion.heaviness", comment: ""),
